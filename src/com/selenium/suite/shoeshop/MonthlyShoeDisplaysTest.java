...
public class MonthlyShoeDisplaysTest extends TestSuiteBase{
...
public int count = -1;
	...
	{
		initialize();
		if(!TestUtil.isTestCaseRunnable(suiteShoeShopXLS, "TestCases", "MonthlyShoeDisplaysTest"))
		{
			isTestSkip = true;
		...
		runModes = TestUtil.getDataSetRunmodes(suiteShoeShopXLS, "MonthlyShoeDisplaysTest");
	}
	
	
	@Test (dataProvider="getTestData")
	public void testMontlyShoeDisplays(String monthName,String browserName,String version,String platform) throws IOException, InterruptedException
	{
		...
		try{
			openBrowser(browserName,version,platform);
	...
	public void updateDataSetStatus() throws IOException
	{
		...
		if(isPass)
		{
			suiteShoeShopXLS.setCellData("MonthlyShoeDisplaysTest", count+2, "Result", "Pass");
		}
		else if(isSkip){
			
			suiteShoeShopXLS.setCellData("MonthlyShoeDisplaysTest", count+2, "Result", "Skipped");
		}
		else if(isFailure){
			
			suiteShoeShopXLS.setCellData("MonthlyShoeDisplaysTest", count+2, "Result", "Fail");
		}
	...
	public void updateTestStatus() throws IOException
	{
		...
		{
			if(suiteShoeShopXLS.getCellData("TestCases", rowNum, 1).equalsIgnoreCase("MonthlyShoeDisplaysTest"))
			{
				row = rowNum;
				break;
			}
	...
	{
		Object[][] data = TestUtil.getTestCaseData(suiteShoeShopXLS, "MonthlyShoeDisplaysTest");
